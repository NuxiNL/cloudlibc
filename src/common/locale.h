...
// Conversion state used by mbtoc32(), mbtowc(), mbsrtowcs(), etc.
struct mbtoc32state {
  unsigned int want;
  char32_t lowerbound;
  char32_t intermediate;
};
static_assert(sizeof(struct mbtoc32state) <= sizeof(__mbstate_t),
...
struct lc_monetary {
  ...
  const char *mon_grouping;          // Grouping size.
...
struct lc_numeric {
  ...
  const char *grouping;          // Grouping size.
};
