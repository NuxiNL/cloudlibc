...
TEST(sigaddset, example) {
  sigset_t set;
  ASSERT_EQ(0, sigemptyset(&set));
  ASSERT_EQ(0, sigaddset(&set, SIGBUS));
  ASSERT_EQ(0, sigaddset(&set, SIGPIPE));
  ASSERT_EQ(0, sigaddset(&set, SIGPIPE));
  ASSERT_EQ(0, sigaddset(&set, SIGSEGV));
  ASSERT_EQ(0, sigaddset(&set, SIGVTALRM));

  // Only some of the standard signals will be set.
